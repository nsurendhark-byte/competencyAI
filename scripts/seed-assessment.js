...
const DB_FILE = path.join(__dirname, '..', 'competency_db.json');

// 100 Technical Questions Array matching exact distribution requirements
const questionsData = [
...
// Seed Function to update competency_db.json
function seedAssessment() {
  console.log('Seeding 100-Question Assessment Suite...');
  ...
  if (fs.existsSync(DB_FILE)) {
    try {
      ...
      console.error('Error reading existing DB file:', e);
    }
  }

  // Clear existing baseline questions
  db.questions = [];
  db.questionOptions = [];
  db.assessments = [];
  db.assessmentQuestions = [];

  // Add 100 questions & 400 options
  ...
  console.log(`Successfully seeded ${db.questions.length} questions and ${db.questionOptions.length} options into competency_db.json.`);
}
...
module.exports = { seedAssessment, questionsData };
